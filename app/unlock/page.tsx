        ...
        <div className="min-h-screen flex items-center justify-center bg-(--background-color) p-4">
            <div className="w-full max-w-md">
                ...
                <form
                    ...
                    className="bg-(--background-secondary) border border-(--border-color) rounded-xl shadow-soft p-6 flex flex-col gap-4"
                >
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-11 pr-12 py-3 bg-(--background-color) border border-(--border-input-color) rounded-lg text-sm text-(--text-color) focus:outline-none focus:border-(--accent-color) transition-colors placeholder-(--text-muted)"
                            />
                            <button
                                type="button"
                                onClick={() => setShow((s) => !s)}
                                className="absolute right-3 top-1/2 -translate-y-1/2 text-(--text-muted) hover:text-(--text-color) text-xs transition-colors"
                                tabIndex={-1}
                            >
                        ...
                        <div className="flex gap-1 h-1 rounded-full overflow-hidden bg-dark-800">
                            {[1, 2, 3, 4].map((lvl) => {
                                ...
                                return (
                                    <div
                                        key={lvl}
                                        className={`w-1/4 ${lvl <= s ? "bg-(--accent-color)" : "bg-(--surface-inactive)"}`}
                                    />
                                );
                            })}
                        </div>
